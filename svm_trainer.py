...
def train_and_save_svms(y_true_sf_gates,save_folder):
    ...
    with open(save_folder+"/all_svm_models_C_0.00000001.pckl", "wb") as f:
        with trange(y_true_sf_gates.shape[0], unit="Indx", desc="Running SVM for gate:") as pbar:
            for i in pbar:
                c1_count = np.sum(y_true_sf_gates[i])
                # If all classes are 0 or all classes are 1s then these two cases are triggered to give atleast one example per class
                if(c1_count == len(y_true_sf_gates[i])):
                    y_true_sf_gates[i][0] = 0
                elif(c1_count == 0):
                    y_true_sf_gates[i][0] = 1
                clf = svm.LinearSVC(C=0.00000001,dual=False,random_state=0,class_weight='balanced')
