...
class Plain_CONV4_Net_BN_pad_k_1_BN(nn.Module):
    def __init__(self, input_channel, seed=2022, num_classes=10):
        ...
        self.conv2_g = nn.Conv2d(128, 128, 3, padding=2)
        self.conv3_g = nn.Conv2d(128, 128, 3, padding=2)
        self.conv4_g = nn.Conv2d(128, 128, 3, padding=2)
...
class Conv4_DLGN_Net_pad_k_1_wo_bn_wo_bias_Small(nn.Module):
    ...
    def init_value_net(self):
        self.conv1_w = nn.Conv2d(self.input_channel, 16, 3, padding=2,bias=False)
        self.conv2_w = nn.Conv2d(16, 16, 3, padding=2,bias=False)
        self.conv3_w = nn.Conv2d(16, 16, 3, padding=2,bias=False)
        self.conv4_w = nn.Conv2d(16, 16, 3, padding=2,bias=False)
        self.pool = nn.AdaptiveAvgPool2d(output_size=(1, 1))
...
def get_model_instance(model_arch_type, inp_channel, seed=2022, mask_percentage=40, num_classes=10, nodes_in_each_layer_list=[], input_size_list=[], pretrained=False, aux_logits=True):
    ...
    elif(model_arch_type == 'plain_pure_conv4_dnn_with_bn'):
        net = Plain_CONV4_Net_BN(inp_channel, seed=seed, num_classes=num_classes)
    elif(model_arch_type == 'plain_pure_conv4_dnn_pad_k_1_st1_with_bn__'):        
        net = Plain_CONV4_Net_BN_pad_k_1_BN(inp_channel, seed=seed, num_classes=num_classes)
    ...
    elif(model_arch_type == 'small_dlgn__conv4_dlgn_pad_k_1_st1_bn_wo_bias__'):
        net = Conv4_DLGN_Net_pad_k_1_wo_bn_wo_bias_Small(inp_channel, seed=seed, num_classes=num_classes)
